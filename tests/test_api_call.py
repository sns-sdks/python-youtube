...
class TestApiCall(unittest.TestCase):
    ...
    def testOAuth(self):
        ...
        token_json = self.api.exchange_code_to_access_token(
            redirect_response, return_json=True
        )
        ...
        refresh_token_json = self.api.refresh_token(
            token.refresh_token, return_json=True
        )
    ...
    def testGetProfile(self):
        ...
        responses.add(responses.GET, self.api.USER_INFO_URL, body=user_info, status=200)
    ...
    def testGetProfileError(self):
        responses.add(
            responses.GET, self.api.USER_INFO_URL, body=HTTPError("Exception")
        )
        with self.assertRaises(pyyoutube.PyYouTubeException):
    ...
    def testRequestError(self):
        responses.add(
            responses.GET, self.BASE_URL + "channels", body=HTTPError("Exception")
        )
        with self.assertRaises(pyyoutube.PyYouTubeException):
            self.api.get_channel_info(
                category_id="GCQmVzdCBvZiBZb3VUdWJl", parts="id,snippet,statistics"
            )
    ...
    def testGetChannel(self):
        ...
        responses.add(
            responses.GET, self.BASE_URL + "channels", json=channel_info_1, status=200
        )
        with open(f"{self.base_path}channel_info_2.json") as f:
            channel_info_2 = json.loads(f.read().encode("utf-8"))
        responses.add(
            responses.GET, self.BASE_URL + "channels", json=channel_info_2, status=200
        )
        with self.assertRaises(pyyoutube.PyYouTubeException):
