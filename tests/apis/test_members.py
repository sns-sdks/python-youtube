...
class ApiMembersTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def testGetMembershipLevels(self) -> None:
        with responses.RequestsMock() as m:
            m.add("GET", self.MEMBERSHIP_LEVEL_URL, json=self.MEMBERSHIP_LEVEL_RES)

            membership_levels = await self.api.get_membership_levels(parts=["id", "snippet"])
            self.assertEqual(
                membership_levels.kind, "youtube#membershipsLevelListResponse"
            )
            self.assertEqual(len(membership_levels.items), 2)

            membership_levels_json = await self.api.get_membership_levels(return_json=True)
